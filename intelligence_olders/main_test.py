...
CONFIG = {
    ...
    # "llm_model": "deepseek-r1-250120",  # LLM模型名称
...
def format_correction_prompt(user_input: str) -> str:
    """
    生成文本纠错提示词（严格仅改错别字，其他不变）
    """
    return f"""【场景说明】这是老年人通过方言语音下单的外卖场景文本，语音识别转换为文本可能不准确：
文本是"来碗惹乎的"，实际上指的是"来碗热乎的"

【纠错规则】
1. **仅修正明确的错别字、同音词错误**（如“香煎机柳”→“香煎鸡柳”），**绝不修改任何正确的字词、语序、语气词或口语化表达**（如“来份”“多放点”“那个”等原封不动保留）。
2. **不添加、删除或调整任何未错误的内容**：即使原文表述不够规范（如“跑的快”），只要无错别字，就保持原样；若有错别字（如“跑的快”→“跑得快”），仅修改错误字“的”→“得”，其他部分不变。
3. 若文本无错别字，**直接返回完全相同的原文本**，不做任何调整。

严格遵循JSON格式，不添加任何额外文本：
...
async def process_correction_with_llm(texts: List[str]) -> List[str]:
    """
    使用LLM对文本列表进行纠错（并发）
    """
    if not texts:
        logger.warning("没有需要纠错的文本")
        return []
    
    # 生成纠错提示词
    prompts = [format_correction_prompt(text) for text in texts]
    
    logger.info(f"开始处理 {len(texts)} 条文本纠错...")
    ...
    for i, result in enumerate(results):
        if isinstance(result, Exception):
            # 记录错误并保留原文本（或根据需求处理）
            logger.error(f"纠错任务 #{i+1} 失败: {type(result).__name__} - {result}")
            corrected_texts.append(texts[i])  # 保留原始文本
            failed += 1
        else:
            # 解析JSON结果
            try:
                ...
                corrected_texts.append(parsed_json)
                successful += 1
    ...
    with open("out_corr.txt", 'w', encoding='utf-8') as f:
        f.write('\n'.join(corrected_texts))
    logger.info(f"纠错完成：{successful} 成功，{failed} 失败")
    return corrected_texts


async def process_audio_with_asr(model: AutoModel, audio_files: List[str]) -> List[str]:
    """
    使用ASR模型处理音频文件
    ...
    logger.info(f"ASR处理完成，提取了 {len(asr_texts)} 条文本,开始纠错了...")
    
    # 调用大模型进行纠错（新增步骤）
    corrected_texts = await process_correction_with_llm(asr_texts)
    
    logger.info(f"纠错完成，得到 {len(corrected_texts)} 条纠正后的文本")
    return corrected_texts

# prompt提示词
def format_llm_prompt(user_input: str) -> str:
    """
    格式化LLM提示词（强化模糊意图解析与食物推断）
    ...
    返回:
        str: 格式化后的提示词
    """
    return f"""请按以下要求解析用户输入文本并返回JSON：
1. 领域意图判断（Call_elm）：
   - 若用户明确或隐含表达点外卖需求（如"点"、"订"、"来份"等关键词，或上下文涉及餐食订购），设为true；否则为false。
   - 例："帮我推荐晚餐" → true，"附近有什么餐厅" → false

2. 食物候选提取（Food_candidate）：
   - 若用户提及具体食物（如"皮蛋瘦肉粥"），直接提取；
   - 若输入模糊但与餐食相关（如"不长胖的"、"热乎的"），输出最可能的食物类别或具体菜品（最多3个候选，用中文逗号分隔）；
   - 若无明确餐食相关内容，返回空字符串""。

严格遵循JSON格式，不添加任何额外文本：
{{
  "Call_elm": boolean,
  "Food_candidate": "具体食物名称或模糊推断结果（多候选用逗号分隔）"
}}
...
#         str: 格式化后的提示词
#     """
#     return f"""识别用户输入文本, 在理解用户输入的前提下，如果存在错别字，对输入文本纠正错别字、同音词等，最后要返回满足要求的JSON。JSON必须包含两个字段: 'Call_elm' 和 'Food_candidate'。
...
async def main_process() -> None:
    ...
    if files_to_process_normally > 0:
        ...
        asr_texts = await process_audio_with_asr(asr_model, normal_filenames)
        ...
        for query_text in food_names:
            if query_text:
                try:
                    ...
                    if result:
                        # 非空结果添加「帮我点」前缀
                        matched_item = result[0].metadata["item_name"]
                        match.append(f"{matched_item}" if matched_item else "")
